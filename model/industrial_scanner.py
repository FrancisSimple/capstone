...
import os
import time
from ultralytics import YOLO  # NEW: YOLO Object Detection
...
# File paths (Dynamic path resolution)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, 'orange_quality_model.h5')
CSV_FILE = os.path.join(BASE_DIR, 'orange_quality_data.csv')

# Initialize CSV if it doesn't exist
if not os.path.exists(CSV_FILE):
    pd.DataFrame(columns=['Timestamp', 'Session_ID', 'Fruit_ID', 'Quality', 'Area', 'Status']).to_csv(CSV_FILE, index=False)
...
def apply_nms(rects, overlapThresh=0.3):
    ...
    while len(idxs) > 0:
        ...
        overlap = (w_inter * h_inter) / area[idxs[:last]]
...
st.markdown("### High-Precision Vision Analysis")
...
if run_conveyor:
    ...
    while run_conveyor:
        ...
        # Classes: 47=Apple, 49=Orange (COCO weights)
        ...
        for r in results:
            boxes = r.boxes
            for box in boxes:
                # YOLO provides perfect box coordinates
                x1, y1, x2, y2 = box.xyxy[0].cpu().numpy().astype(int)
                w = x2 - x1
                h = y2 - y1
                # Filter out tiny "scattered" noise
                if w > 40 and h > 40:
        ...
        for (x, y, w, h) in clean_rects:
            ...
            for o_id, data in st.session_state.current_objects.items():
                ...
                if dist < 150: # Wide threshold to prevent Stationary Box issue
                    match_id = o_id
                    break
            ...
            if match_id is None:
                ...
                # [cx, cy, quality, color, status, last_seen]
                st.session_state.current_objects[match_id] = [cx, cy, 0, (255, 255, 255), "SCANNING", time.time()]


            
            st.session_state.current_objects[match_id][0] = cx
            st.session_state.current_objects[match_id][1] = cy
            st.session_state.current_objects[match_id][5] = time.time() # Update last_seen
            active_now.append(match_id)

            
            if match_id not in st.session_state.logged_ids:
                roi_x1, roi_y1 = max(0, x-15), max(0, y-15)
                roi_x2, roi_y2 = min(frame.shape[1], x+w+15), min(frame.shape[0], y+h+15)
                ...
                if roi.size > 0:
                    ...
                    st.session_state.current_objects[match_id][4] = status

                    
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    pd.DataFrame([[timestamp, st.session_state.session_id, match_id, round(quality, 1), int(w*h), status]]).to_csv(CSV_FILE, mode='a', header=False, index=False)
            ...
            stored_s = st.session_state.current_objects[match_id][4]
            
            # Draw on the original frame
            cv2.rectangle(frame, (x, y), (x+w, y+h), stored_c, 3)
            label = f"{stored_q:.0f}% ({stored_s})"
        ...
        # CLEANUP: Only remove objects if they've been missing for more than 2 seconds
        now = time.time()
        st.session_state.current_objects = {
            k: v for k, v in st.session_state.current_objects.items() 
            if k in active_now or (now - v[5]) < 2.0
        }
        ...
        # Dashboard Refresh (Filtered for current session)
        if os.path.exists(CSV_FILE):
            df_logs_full = pd.read_csv(CSV_FILE)
            df_session = df_logs_full[df_logs_full['Session_ID'] == st.session_state.session_id]
        else:
            df_session = pd.DataFrame(columns=['Fruit_ID', 'Quality', 'Status']) # Safety


        
        live_metric_total.metric("Total Scanned", len(st.session_state.logged_ids))
        reject_count = len(df_session.query('Status == "REJECT" or Status == "LOW"'))
        live_metric_rejects.metric("Rejects Found", reject_count, delta_color="inverse")
        
        live_table.table(df_session.tail(8)[['Fruit_ID', 'Quality', 'Status']].iloc[::-1])
